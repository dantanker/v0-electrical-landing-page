            ...
            {({ ready, size, position, duration }) => (
              <div className="relative">
                <div
                  ...
                  className={clsx(
                    { hidden: !ready },
                    "absolute inset-y-0 left-0 h-full rounded-lg bg-white/10 transition-[width,transform]"
                  )}
                />
            ...
            <nav className="flex flex-col gap-2 mb-4" aria-label="Mobile navigation">
              {navItems.map((item) => (
                <button
                  ...
                  className="text-slate-300 hover:text-white transition-colors font-medium py-2 text-left px-2"
                >
                  {item.label}
                </button>
              ))}
